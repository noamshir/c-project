...
/* FIRST PASS */
int handle_command_line_first_pass(symbol_item **symbol_table, char *line, char ***array_of_commands, int *IC)
{
    ...
    if (is_label_declaration(label))
    {
        ...
        if (!add_symbol_item(symbol_table, label_name, "code", *IC, 0))
        {
            return 0;
        }
        command = strtok(NULL, " ");
    ...
    case COMMAND_SUB:
    case COMMAND_LEA:
        return handle_two_op_line_first_pass(command_index, rest_of_line, array_of_commands, IC);
        break;
    case COMMAND_CLR:
    case COMMAND_NOT:
    case COMMAND_INC:
    ...
    case COMMAND_RED:
    case COMMAND_PRN:
        return handle_one_op_line_first_pass(command_index, rest_of_line, array_of_commands, IC);
        break;
    case COMMAND_RTS:
    case COMMAND_STOP:
        return handle_no_op_line_first_pass(command_index, rest_of_line, array_of_commands, IC);
        break;
    default:
        print_error(PROCESS_ERROR_INVALID_COMMAND);
        return 0;
...
int handle_no_op_line_first_pass(int command_index, char *str, char ***array_of_commands, int *IC)
{
    ...
    return handle_op_line_first_pass(command_index, NULL, NULL, array_of_commands, IC);
}

int handle_one_op_line_first_pass(int command_index, char *str, char ***array_of_commands, int *IC)
{
    char *dst;
    ...
    return handle_op_line_first_pass(command_index, NULL, dst, array_of_commands, IC);
}

int handle_two_op_line_first_pass(int command_index, char *str, char ***array_of_commands, int *IC)
{
    char *src, *dst;
    ...
    printf("src: %s, dst: %s\n", src, dst);
    return handle_op_line_first_pass(command_index, src, dst, array_of_commands, IC);
}

int handle_op_line_first_pass(int command_index, char *src, char *dst, char ***array_of_commands, int *IC)
{
